...
export default defineComponent({
  ...
  computed: {
    ...
    customVideo(): object | null {
      if (this.pageData.featured.heroBlocks) {
        if (this.pageData.featured.heroBlocks.length > 0) {
    ...
    customImage(): object | null {
      ...
      if (this.pageData.featured.heroBlocks) {
        if (this.pageData.featured.heroBlocks.length > 0) {
          ...
          if (
            block.blockType === 'ImageChooserBlock' ||
            block.blockType === 'HeroImageBlock'
          ) {
            return block.image
