<template>
  <div v-if="data" class="-nav-offset">
    ...
    <YearTicker :target-year="currentYear" />
        ...
        >
          <img
            ...
            :style="{
              ...
              '--x-translate': `${
                (index % 2 === 0 ? 180 : -130) + 10 * (index % 3)
              }%`,
            }"
            alt=""
          />
        </ParallaxElement>
        <BlockMilestone :data="milestone" :handle-click="showMilestone" />
      </div>
    ...
    <BackToTop
      ...
      @click="
        jumpTo = ''
        currentYear = sortBy === 'oldestDate' ? oldestYear : latestYear
      "
    />
...
import BlockMilestone, {
  Milestone,
} from '@/components/PageTimeline/BlockMilestone.vue'
...
export default defineComponent({
  ...
  watch: {
    sortBy(newSortBy) {
      this.currentYear =
        newSortBy === 'oldestDate' ? this.oldestYear : this.latestYear
  ...
  created() {
    ...
    this.oldestYear =
      this.data.milestones[this.data.milestones.length - 1].date.split('-')[0]
    this.currentYear =
      this.sortBy === 'oldestDate' ? this.oldestYear : this.latestYear
  ...
  methods: {
     ...
     */
    observeYears() {
      const years = this.$el?.querySelectorAll<HTMLDivElement>(
        '[data-milestone-year]'
      )
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          ...
          if (intersections.length > 0) {
            this.currentYear = (intersections[0].target as HTMLDivElement)
              .dataset.milestoneYear as String
          }
        },
        {
          threshold: 1,
