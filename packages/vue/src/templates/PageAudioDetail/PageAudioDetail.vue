<template>
  ...
  >
    ...
    />
    <meta v-if="data.summary" itemprop="description" :content="data.summary" />
    ...
    <LayoutHelper indent="col-2" class="lg:pt-12 pt-4">
      <div class="lg:grid flex flex-col grid-cols-10 mb-12">
        <div
          v-if="
            data.summary ||
            data.transcript ||
            (data.relatedLinks && data.relatedLinks.length)
          "
          class="lg:order-1 order-2 col-span-7 mb-12"
        >
          <BlockText :text="data.summary" />
          <hr v-if="data.summary" class="border-gray-light-mid lg:my-8 my-5" />
          <div
          ...
          <template v-if="data.relatedLinks && data.relatedLinks.length">
            <BlockRelatedLinks class="mt-12" :data="data.relatedLinks[0]" />
          </template>
        </div>
        <aside
          class="audio-aside lg:order-2 order-1 col-start-9 col-end-11 mb-12"
        >
          <div class="mt-5 lg:mt-0 lg:mb-12">
            <div class="lg:w-auto w-full">
              ...
              >
                <template #modalTrigger>
                  <BaseButton class="w-full mb-5" variant="primary" compact>
                    Subscribe
                  </BaseButton>
                </template>
                <template v-if="data.subscribeLinks[0].heading" #modalHeader>
                ...
                <div class="SubscribeModal mb-8">
                  ...
                  <ul
                    v-if="
                      data.subscribeLinks[0].links &&
                      data.subscribeLinks[0].links.length > 0
                    "
                    class="text-body-md mb-5"
                  >
                    <li
            ...
            <div class="lg:w-auto w-full">
              <BaseButton
                ...
                :variant="
                  data.subscribeLinks && data.subscribeLinks.length
                    ? 'secondary'
                    : 'primary'
                "
                compact
