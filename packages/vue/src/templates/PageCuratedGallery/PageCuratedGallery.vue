  ...
  <div v-if="data" id="galleryTop" class="PageCuratedGallery ThemeLight">
    ...
    <LayoutHelper class="lg:mt-8 lg:mb-4 mt-5 mb-0">
      ...
      <LayoutHelper
        indent="col-2"
        :class="
          data.firstPublishedAt && data.firstPublishedAt.length > 2
            ? ''
            : 'py-4 mb-2'
        "
      >
        ...
        <div class="flex flex-row ml-auto py-3 lg:py-0 justify-between">
          ...
          >
            <button
              ...
              @click="
                $store
                  ? $store.commit('curatedGalleries/UPDATE_VIEW', true)
            ...
            <button
              ...
              "
              :aria-label="
                !showGridView ? 'Slideshow View (enabled)' : 'Slideshow View'
              "
              :disabled="!showGridView"
              @click="
                $store
                  ? $store.commit('curatedGalleries/UPDATE_VIEW', false)
    ...
    <template v-if="data.galleryType">
      <div :class="'gallery-' + data.galleryType" class="mx-auto bg-white">
        <div v-if="data.galleryItems" class="container mx-auto">
          <div v-if="showGridView" class="CuratedGallery-thumbnails">
          ...
          <div v-else class="CuratedGallery-carousel">
          ...
          <LayoutHelper indent="col-2">
            ...
            >
              <template
                v-if="
                  data.galleryDownloadLink && data.galleryDownloadLink.length
                "
              >
          ...
          <template v-if="data.body">
            <BlockStreamfield class="lg:mb-10 mb-4" :data="data.body" />
          </template>
          <LayoutHelper indent="col-2">
            <!-- related links -->
...
export default defineComponent({
  ...
  computed: {
    showGridView() {
      if (
        this.$store &&
        this.$store.state.curatedGalleries.showGridView !== null
      ) {
        return this.$store.state.curatedGalleries.showGridView
      }
