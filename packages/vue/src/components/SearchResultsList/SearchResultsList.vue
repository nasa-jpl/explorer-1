<template>
  ...
  >
    <li
      ...
      :key="page.id"
      :class="{
        'md:col-span-6 lg:col-span-4 xl:col-span-3 col-span-12 gap-5':
          grid && !condenseGrid,
    ...
    >
      <template v-if="!grid">
        <SearchResultCard
          ...
          :featured="
            featureFirstResult ? index === 0 && currentPage === 1 : false
          "
          heading-level="h2"
        />
      </template>
      ...
      return this.pages
        ...
        .map((page: Page) => {
          ...
          if (handle === 'events_eventpage') {
            date =
              'Event date: ' +
              parseDate(page._source[handle + '__start_datetime'])
