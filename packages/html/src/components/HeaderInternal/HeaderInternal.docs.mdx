...
> In all of the above examples, `allowSectionBreaks` and `allowSectionColumns` can be modified as needed. For information on what these are, see documentation on the Internal Navigation [Panel](?path=/story/global-layout-headers-for-internal-sites-elements-panel--base-story)
