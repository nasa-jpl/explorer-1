    ...
    for (const [index, item] of menuItems.entries()) {
      ...
      menuItemsTemplate += `<li class="${item.isActive ? 'active' : ''}">
        ${
          isMegaMenu
            ? `
            <button class="toggle-NavInternalPanel flex flex-nowrap items-center w-full lg:w-auto font-text tracking-[-.03rem]"
          ...
          `
            : `
          <a href="${item.url}" class="block font-text tracking-[-.03rem]"${
