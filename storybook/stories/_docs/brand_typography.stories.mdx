...
<Meta title="Brand Identity/Typography" parameters={{ viewMode: 'docs' }} />
...
Archivo Narrow is used for subtitles and buttons and always in UPPERCASE.
...
Here are colors intended for text, depending on the background. Note some of those colors will not pass contrast requirements if used on a different background.
