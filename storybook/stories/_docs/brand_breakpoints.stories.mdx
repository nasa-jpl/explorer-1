...
<Meta
  title="Brand Identity/Responsive Design"
...
> The examples on this page illustrate breakpoints based on viewport size. Try changing your browser size to see how the examples adapt.
...
We set our breakpoints and associated containers via the `screens` and `containers.screens` options in our [TailwindCSS configuration](?path=/docs/appendix-tailwindcss--page#theme). These containers are used in conjunction with our [Grid](?path=/docs/brand-identity-grid-and-layout--base-grid) to create the site and page layouts, all the way down to the component level.
          ...
          <div class="bg-gray-light border">
            ...
            <div class="max-w-screen-3xl mx-auto bg-black text-white">
              <code>.max-w-screen-3xl</code>
                <div class="lg:BaseGrid container mx-auto bg-gray-light text-black">
...
The site uses `3xl` as its maximum width. Pages and full-bleed components follow this rule. No content ever exceeds 3xl, although visual layout elements may (i.e. background colors)
...
Screen tokens are used with TailwindCSS classes to control widths:
